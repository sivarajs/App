...
var urlLoader = {
	...
	loadXHtml : function(url, parent, mode) {
		...
		if (!url.startsWith("/")) url = APP_HOME+url;
...
(function() {
			...
			document.nodValue = function(node) {
				var nds = node.childNodes;
				for (var i=0, nd; nd = nds[i++];) {
					if (nd.nodeType == 3) {
						return nd.nodeValue;
...
UIComp.prototype.position = function(parent) {
	...
	if (parent != undefined) {
		...
		//		
		// x = (x + this.width - scrollLeft > docWidth) ? x - this.width : x //
...
var workArea = (function() {
	...
	WR.load = function(title, url) {
		urlLoader.attachXHtml(url, "workArea");
	}
...
THEME_HOME = FW_HOME + "theme/" + THEME_NAME + "/";
